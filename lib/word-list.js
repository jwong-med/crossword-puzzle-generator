...
class WordList {
  ...
  _loadCurated() {
    for (const word of CURATED_WORDS) {
      ...
      if (/^[A-Z]+$/.test(w) && w.length >= 3 && w.length <= 15) {
        if (!this._byLength.has(w.length)) {
          this._byLength.set(w.length, new Set());
        }
        this._byLength.get(w.length).add(w);
      }
    }
  }

  static _isQualityWord(w) {
    ...
    if (w.length >= 4 && vowelCount < w.length / 4) return false;
    ...
    for (const ch of w) {
      if (vowels.has(ch)) {
        ...
        consecConsonants++;
        if (consecConsonants >= 4) return false;
      }
    }
    ...
    for (const ch of w) {
      if (vowels.has(ch)) {
        consecVowels++;
        if (consecVowels >= 3) return false;
      } else {
        consecVowels = 0;
      }
    }
   ...
   * @returns {string[]}
   */
  matchPattern(pattern, exclude) {
    const length = pattern.length;
    const candidates = this._byLength.get(length);
    ...
    const curatedSet = new Set(
      CURATED_WORDS.filter(w => w.length === length).map(w => w.toUpperCase())
    );

    const matchesCurated = [];
    const matchesDict = [];
    for (const word of candidates) {
      if (excl.has(word)) continue;
      if (regex.test(word)) {
        if (curatedSet.has(word)) {
          matchesCurated.push(word);
        } else {
          matchesDict.push(word);
        }
      }
    }

    return matchesCurated.concat(matchesDict);
  }

  hasWordsOfLength(length) {
    const s = this._byLength.get(length);
    return s !== undefined && s.size > 0;
  }

  wordCount() {
    let total = 0;
    for (const words of this._byLength.values()) {
      total += words.size;
    }
    return total;
  }
